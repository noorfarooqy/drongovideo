$(document).ready(function(event) {
    ...
    //     volume: 0.9
    // });
    $('body').attr('stream', 'false');
    $('#dlg').dialog('close');
    $('#invite').dialog('close');
    $('#ancStartChat').bind('click', function() {
        if(!MediaIsAvailable())
        {
            alert('Please Enable Audio And Video');
        }
        else
        {
    ...
    */
    ...
    function setupWebRTC() {
        ...
        // connection.socketURL = window.user.socketURL+":443/";
        ...
        // Local video is disabled by default
        //local audio is enabled by default
        connection.extra.enabledVideo = false;
        connection.extra.isAudioMuted = false;
        console.log(connection);
        connection.onMediaError = function(event) {
            $('.wrapLoader').show();
            $.messager.alert('Alert', 'Sorry. Please ensure your webcam and audio devices are available');
            $('.wrapLoader').hide();
...
//        adjustBigVideo();
    ...
    $("button#create_meeting_room").bind("click", function(e) {
        ...
        room_id = $(this).attr('data');//window.user.roomId;//'1322156';//$("#room_id").val();
        ...
        user_name = window.user.userName;//'Noor ';//$("#user_name").val();
    ...
    */
                    ...
                    $('#close_class').css('display', 'block');
                    $('#close_class_room').attr('data', $(this).attr('data'));
                    $('#close_class_room').text('Stop '+lesson_name);
    ...
    $("button#join_meeting_room").bind("click", function(e) {
        ...
        user_name = window.user.userName;//'Abdi';//$("#user_name").val();
        if (user_name.length < 4) {
            $("#user_name").textbox({
                value: "",
                prompt: "Enter your name (At least 4 characters)"
        ...
        if(!MediaIsAvailable())
        {
            $.messager.alert('Alert', 'Please ensure your webcam and audio devices are available!');
            $('.wrapLoader').hide();
            return;
        }
    ...
    var observer = new MutationObserver(function(mutations) {
      mutations.forEach(function(mutation) {
        ...
        if (mutation.type == "attributes" && mutation.attributeName=="stream") {
    ...
    function sendDocStream()
    {
        console.log('should start sending');
            
        var targetUrl = $('.pdfMoveDiv').attr('target');
        firstSend = {
            screenImage:false,
            dataType : 'streamDocument',
            document : {document: targetUrl}
        };
        console.log('first send : ', firstSend);
        connection.extra.docStream = true;
        connection.send(firstSend);
        var docStreamInterval = setInterval(function(){
            var pdfDiv = document.querySelector('.pdfMoveDiv');
            secondSend = {docProperties: {topOffset: pdfDiv.offsetTop, leftOffset: pdfDiv.offsetLeft}};
            connection.send(secondSend);
            console.log('sent: ', secondSend);
        }, 1000);
    ...
    function updateContainerUserFaces( ) {
        ...
        connection.getAllParticipants().forEach(function(userid) {
            ...
            if (remotePeer.extra.enabledVideo && remotePeer.streams.length != 0) {
                // if(window.user.userType !== "Tutor")
                // {
                //     console.log('remote stream: ',connection )
                //     var remoteVideoStream = remotePeer.streams[0].id;
                //     $('#my_video').attr('src',connection.streamEvents[remoteVideoStream].blobURL);
                //     $('#my_video').play();
                //     $('.tutor_thumbnail').css('display', 'block');
                //     console.log('should display tutor video');
                // }
                // else
                //     console.log('sending video to student');
                var remoteVideoStream = remotePeer.streams[0].id;
                var remoteStream = connection.streamEvents[remoteVideoStream];
    ...
    $('#input-box').textbox('textbox').bind('keydown', function(e) {
        if (e.keyCode == 13) {
            ...
            div.innerHTML = msg;
            chatContainer.appendChild(div);
            connection.send({msg:msg,messageType:'chat'});
    ...
    $("#btnShareVideo").bind("click", function(event) {
        console.log('Connection enabled video: ', connection.extra.enabledVideo);
        console.log('Connection enabled Audio : ', connection.extra.isAudioMuted);
        connection.extra.enabledVideo = !connection.extra.enabledVideo;
        if (connection.extra.enabledVideo) {
            console.log('Video is enabled inside if');
        ...
        if(window.user.userType === "Tutor")
        {
            ...
            console.log('Adding video to bigger one');
            containerBigVideo.html(localMediaElement);
            currentBigVideo.streamid = localStream.streamid;
            currentBigVideo.enabled = true;
            if( document.getElementById(localStream.streamid) ){
    ...
    connection.onmessage = function onMessageHandler(event) {
        ...
        if (event.data && event.data.hasOwnProperty('screenImage')) {
            ...
            if(event.data.hasOwnProperty('userType'))
            {
                console.log('Data from ', event.data.userType);
                if(event.data.userType === "Tutor")
                {
                    setTimeout(function() {
                        connection.getAllParticipants().forEach(function(userid) {
                                            var remotePeer = connection.peers[userid];
                                            ...
                                            var remoteVideoStream = remotePeer.streams[0].id;
                                            var remoteStream = connection.streamEvents[remoteVideoStream];
                                            console.log( "Remote Stream From Tutor" );
            ...
            $("#btnShareAudio").trigger("click");
            return;
        }else if(event.data.hasOwnProperty('msg')) {
            var div = document.createElement('div');
            div.innerHTML = event.data.msg || event;
            chatContainer.appendChild(div);
        }
        // console.log(event);
        // console.log(connection);
    ...
    connection.streamended = connection.onclose = function(event) {
        ...
        var msg = '<b> System: </b> > Connection closed';
        div.innerHTML = msg;
        chatContainer.appendChild(div)
        chatContainer.scrollTop = chatContainer.lastChild.offsetTop;
    };
...
function openTcLogin() {
    ...
    $('.subProperties').hide();
    $win.window('open');
    $('#sign').window('refresh', '/whiteboard/getContents');
} 
...
function MediaIsAvailable()
{
    ...
    if(!navigator.mediaDevices.getUserMedia({video:user}))
        return false;
    else 
        return true;
}
